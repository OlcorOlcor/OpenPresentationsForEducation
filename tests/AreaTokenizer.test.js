...
test("Opening and closing tag back to back", () => {
    expect(tokenizeText(simpleOpenTagText + simpleCloseTagText)).toEqual([simpleOpenTag, simpleCloseTag]);
});

test("Text between tags", () => {
    expect(tokenizeText(simpleOpenTagText + "Text in between" + simpleCloseTagText)).toEqual([simpleOpenTag, "Text in between", simpleCloseTag]);
})

test("< in plain text", () => {
    expect(tokenizeText("There's a < in my text")).toEqual(["There's a < in my text"]);
});
...
test("Complex", () => {
    expect(tokenizeText("Hello World" + simpleOpenTagText + "text with < and >" + simpleCloseTagText + openTagText2 + closeTagText2 + "Final words"))
        .toEqual(["Hello World", simpleOpenTag, "text with < and >", simpleCloseTag, openTag2, closeTag2, "Final words"]);
});